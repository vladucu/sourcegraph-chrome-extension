...
function main(){
	...
	//checkLanguage to addSearchButton
...
function handleBackButton(){
	try{document.getElementById('sg-search-button-container').addEventListener("click", searchButtonClick)}catch(err){};
...
function checkLanguageNotAjax(){
		...
		if (fileElem){
			...
			if (repoElem[0].innerText.toLowerCase() === "go") {
				repoIsGo=true;
				return;
			}
		}
...
function addSearchButton (){
	...
	document.getElementById('sg-search-button-container').addEventListener("click", searchButtonClick);
}
	...
	if (dataArray.SearchResults){
		...
		for (var i = 0; i < dataArray.SearchResults.length; i++) {
			...
			if (filename!==prevFile){
				$('.code-list').append("<div class='code-list-item code-list-item-public repo-specific'> <span class='language'>" +filetype+ "</span> <p class='title'><a href='https://sourcegraph.com/github.com/"+user+"/"+repo+"@"+branch+"/.tree/"+filename+"#L"+lineNumber+"' title='"+filename+"' target='none'>"+filename+"</a><br><span class='text-small text-muted match-count'> Results in "+filename+"</span></p><div class='file-box blob-wrapper'><table><tbody class='textres'></tbody>"); for(var j = 0; j < contentArray.length; j++) {try{$('.textres:last').append(" <tr> <td class='blob-num'> <a href='https://sourcegraph.com/github.com/"+user+"/"+repo+"@"+branch+"/-/tree/"+filename+"#L"+lineNumber+"' target='none'>"+lineNumber+"</a> </td> <td class='blob-code blob-code-inner'> "+contentArray[j]+" </td> </tr>")}catch(err){"Weird JS append error"}
